...
public class ReceiptDAO {
    ...
    public ArrayList<ReceiptDTO> selectAll() {
		...
		try {
			...
			while(rs.next()) {
				...
				ReceiptDTO p = new ReceiptDTO(maPN, ngayTao, nguoiTao, maNCC, tongTien);
				ketQua.add(p);		
			}
		} catch (Exception e) {
			e.printStackTrace();               
	...
	public ReceiptDTO selectById(ReceiptDTO pn) {
		...
		try {
			...
			while(rs.next()) {
				...
				String maNCC = rs.getString("mancc");
				int tongTien = rs.getInt("tongtien");
				ketQua = new ReceiptDTO(maPN, ngayTao, nguoiTao, maNCC, tongTien);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public int insert(ReceiptDTO pn) {
		...
		try {
			Connection con = KetNoiDB.getConnection();
			String sql = "insert into receipt(mapn,ngaytao,nguoitao,mancc,tongtien) value(?,?,?,?,?)";
			...
			statement.setString(1, pn.getMaPN());
                        ...
                        statement.setString(4, pn.getMaNCC());
                        statement.setInt(5, pn.getTongTien());
